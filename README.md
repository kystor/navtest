...
这是一个专为 **容器化环境** 设计的轻量级 Shell 脚本。它不仅能将本地文件的变化推送到 GitHub，还能**自动拉取云端的更新**覆盖本地。

特别针对 **SQLite 数据库** 进行了优化，解决了 Docker 挂载卷中常见的 `Read-only database` 权限问题。

## ✨ 核心特性

* **⚡️ 双向同步**：
    * **上行**：监控本地文件变化 -> 自动 Push 到仓库。
    * **下行**：启动或轮询时检查远程仓库 -> 自动 Pull 并覆盖本地（适合多地部署场景）。
* **🛡️ SQLite 专属保护**：
    * **权限修复**：每次操作前后自动执行 `chmod`，确保数据库文件和目录始终可读写。
    * **Inode 保护**：使用 `cat > file` 写入数据，而非直接 `cp` 替换，完美保留原文件属性，防止数据库锁死。
* **⏳ 防抖动机制**：内置等待机制，确保文件写入完全结束后再触发同步。
* **🐳 容器友好**：支持 `BACKUP_REPO_URL` 完整链接配置，配置更简单。
...
你可以在 `docker-compose.yaml` 中设置以下变量：

### 🔴 必填项 (二选一)

你可以选择 **方案 A (推荐)** 或 **方案 B** 来配置仓库信息。
...
### 推荐：Docker Compose (Sidecar 模式)

将此脚本作为“Sidecar（边车）”容器运行，与主应用共享数据卷。
...
services:
  ...
  nav-app:
    ...
    restart: always
    volumes:
      - ./data:/app/database # 映射本地目录到容器

  # 2. 备份与同步服务
  backup-service:
    image: alpine/git:latest  # 使用包含 git 的轻量级镜像
    container_name: nav-sync
    restart: always
    volumes:
      - ./data:/app/database   # 关键：必须挂载与主应用相同的目录
      - ./backup.sh:/app/backup.sh # 挂载脚本文件
    entrypoint: ["/bin/sh", "/app/backup.sh"] # 覆盖启动命令执行脚本
    environment:
